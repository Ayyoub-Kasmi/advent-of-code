...
var roundOutcome = function (shapeOne, shapeTwo) {
...
var roundsPromise = (0, utils_1.processInputFile)("input.txt", function (rl) { var _a, rl_1, rl_1_1; return __awaiter(void 0, void 0, void 0, function () {
    var result, line, e_1_1;
    var _b, e_1, _c, _d;
        ...
        switch (_e.label) {
            ...
            case 3:
                ...
                try {
                    ...
                    result.push(line.split(' ').map(function (value) {
...
roundsPromise.then(function (rounds) {
    // //part one solution
    // followPreviousStategyGuide(rounds);
    // part two solution
    followNewStrategyGuide(rounds);
});
function chooseShape(opponentShape, myRoundOutcome) {
    if (myRoundOutcome === "DRAW")
...
function followNewStrategyGuide(rounds) {
    var totalScore = 0;
    rounds.forEach(function (round) {
        var opponentMove = round[0];
        var roundStatus = newCode[round[1]];
        if (isValidMove(opponentMove)) {
            var opponentShape = opponentCodes[opponentMove];
            var myShape = chooseShape(opponentShape, roundStatus);
            var outcome = roundOutcome(myShape, opponentShape);
            var score = roundScore[outcome] + moveScore[myShape];
...
function followPreviousStategyGuide(rounds) {
    var totalScore = 0;
    rounds.forEach(function (round) {
        var opponentMove = round[0];
        var myMove = round[1];
