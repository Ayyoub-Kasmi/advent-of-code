...
var streamPromise = (0, utils_1.processInputFile)("input.txt", function (rl) { var _a, rl_1, rl_1_1; return __awaiter(void 0, void 0, void 0, function () {
...
streamPromise.then(function (stream) {
    ...
    while (!(comparisonResult = checkDistinctChars(stream, amount, currentIndex)).success && currentIndex < stream.length)
        currentIndex = comparisonResult.firstDuplicateIndex + 1;
    console.log('part two marker: ', currentIndex + 14);
});
function checkDistinctChars(data, amount, startIndex) {
    var result = {
        success: false,
        firstDuplicateIndex: -1
    };
    for (var i = startIndex; i < startIndex + amount; i++) {
        for (var j = i + 1; j < startIndex + amount; j++) {
            if (data[i] === data[j]) {
